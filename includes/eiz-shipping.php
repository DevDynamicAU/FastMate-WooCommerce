	...
	class Eiz_Shipping_Method extends WC_Shipping_Method {
		...
		*/
		...
		public function generate_subscribe_html($key, $data) {
			$defaults = array(
				...
				'custom_attributes' => array(),
				'desc_tip' => true,
				'description' => 'Configure couriers availability',
				'title' => 'Couriers',
			);
		...
		public function generate_couriers_html($key, $data) {
			...
			if (!class_exists('Eiz_Couriers_API')) {
				// Include Eiz Couriers API
				include_once 'eiz-couriers-api.php';
			}
		...
		*/
		public function calculate_shipping($package = array()) {
			...
			try {
				...
				$preferred_rates = Eiz_Shipping_API::getShippingRate($package);
				//print_r($preferred_rates);exit;
			} catch (Exception $e) {
		...
		*/
		protected function defaultDimension($length) {
			// todo return the default dimension from the settings
